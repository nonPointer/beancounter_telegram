...
INVEST_ORDER_SYSTEM_PROMPT = (
    ...
    "For SELL orders: "
    "  - Credit (negative) the holding account using @@ with the result (P&L) from the screenshot: "
    "    -QUANTITY TICKER @@ RESULT_AMOUNT RESULT_CURRENCY  ; result from screenshot "
    "    Do NOT calculate or look up cost basis. Use ONLY the result/P&L figure shown in the screenshot. "
    "  - Add a capital gain/loss posting to an Income account: "
    "    Income:Investments:PnL  -RESULT_AMOUNT RESULT_CURRENCY "
    "    (negative value for a gain, positive value for a loss) "
    "  - If there is a fee, add a separate Expenses posting. "
    "  - Debit the cash/settlement account for net proceeds (the actual cash credited, from the screenshot). "
    ...
    "\n"
    "Example SELL with capital gain (result = 150.00 USD from screenshot):\n"
    "2026-03-06 * \"Trading 212\" \"Sell 10 AAPL (Apple)\"\n"
    "  Assets:Broker:AAPL       -10 AAPL @@ 150.00 USD  ; result from screenshot\n"
    "  Income:Investments:PnL   -150.00 USD\n"
