...
分析按需探测后端模型的工具调用及结果回传能力，不需要开关。明确支持或不支持的结果缓存一小时；不支持时在当前 endpoint 自动使用 JSON 查询协议，切换协议不占用分析轮次。探测结果不明确、探测报错、超时或预算耗尽时，也先在当前 endpoint 使用 JSON，仅本次降级，不缓存为不支持。JSON 请求仍失败时才按顺序尝试下一后端；原生工具分析请求中的鉴权、限流及连接错误仍按顺序尝试下一后端。
